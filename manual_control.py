...
class CarlaGame(object):
            ...
            while True:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        return
                # self.img_array = self._on_render()
                self._on_loop()
                self._on_render()
                self. count += 1
        finally:
    ...
    def _on_loop(self):
        ...
        #     sem_img = self._mini_view_image2.return_data()
        #     # if self.count <=15:
        #     #     plt.imshow(sem_img)
        #     #     plt.show()
        #     print("IMAGE2 is not none")
        # else:
        #     print("None SemSeg")
        ...
        # elif self._enable_imitation and self.count>0:
            # control = measurements.player_measurements.autopilot_control
        #     # if self.count < 10:
        #     #     plt.imshow(self.img_array[:,:,0],cmap='tab20',aspect='auto')
        #     #     plt.show()


        #     print("Shape is" + str(np.shape(np.asarray(self.img_array))))


            # img = np.asarray(self.img_array)
            # np.reshape(img, img.shape + (1,))

        #     print("Shape of image is " +str(np.shape(img)))

        #     # new_img = cv2.resize(sem_img,shape=(180,320))
        #     # control.steer = float(self.model.predict(cv2.resize(self.img_array,shape=(180,320))))
        #     control.steer = float(self.model.predict(img[None,:,:,0],batch_size=1))
        #     print("Shape no 2 is " + str(np.shape(np.asarray(img))))

        #     print("Predicting Steer")

        #     print("Steer value is " + str(control.steer))
        #     self.client.send_control(control)
        #     # #self.client.send_control(measurements.player_measurements.autopilot_control)
        #     # #control.steer= 0.0
        #     # img_array = np.asarray(self._on_render())
        #     # # print("Shape of sem seg is " + str(np.shape(img_array)))
        #     # if img_array != []:
                # control.steer =float(self.model.predict(img_array, batch_size=1))
        #     # else:
        #     #     control.steer= 0.0
        #     # self.client.send_control(control)
        #     # print("Steer value is " + str(control.steer))          
    ...
    def _on_render(self):
        ...
        if self._mini_view_image2 is not None:
            ...
            if self._enable_imitation:
                ...
                # # try:
                control.steer =float(self.model.predict(img[:,:,:], batch_size = 1))/10.0
        ...
        # if np.shape(np.asarray(temp_array)) == (240,320,1):
        #     plt.imshow(temp_array[:,:,0])
        #     plt.show()

        # print("Shape is" + str(np.shape(np.asarray(temp_array))))
        # return temp_array
        # return sem_return
